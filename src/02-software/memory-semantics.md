...
2. GMMU 接收虚拟地址 (VA)
   │
   ▼
3. TLB (Translation Lookaside Buffer) 查询
   │
   ├── 命中 ──► 直接获取物理地址，跳到步骤6
   │
   └── 未命中 ──► 继续页表遍历
...
3. 建立P2P映射
   │
   ├── 分配 Peer ID：kbusGetPeerId_HAL()
   │   └── 每个GPU为其peer分配本地唯一ID (0~7)
   │
   ├── 创建映射：kbusCreateP2PMapping_HAL()
   │   └── 配置硬件路由表，使Peer ID指向正确的物理链路
   │
...
4. 配置页表 (PTE)
   ...
   │   └── 设置物理地址 = 远程GPU显存地址
   │
   └── PTE写入（参见 `nv_gpu_ops.c` 中的PTE构建逻辑）：
...
```
┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐
│  SM/CE   │  │   GMMU   │  │  NVLink  │  │ NVSwitch │  │ 远程GPU  │
└────┬─────┘  └────┬─────┘  └────┬─────┘  └────┬─────┘  └────┬─────┘
     │             │             │             │             │
     │ LD/ST VA    │             │             │             │
     │────────────►│             │             │             │
...
│  导出节点                                 导入节点                           │
│                                                                             │
│  RM_API->Alloc(NV_MEMORY_FABRIC)                                            │
│     │                                                                       │
│     ▼                                                                       │
│  RM_API->Alloc(NV_MEMORY_EXPORT)                                            │
│     │                                                                       │
│     │  生成 exportPacket:                                                   │
│     │   - uuid[16]                                                          │
│     │   - nodeId                                                            │
...
│     │                                          │                            │
│     │                                          ▼                            │
│     │                          RM_API->Alloc(NV_MEMORY_FABRIC_IMPORT_V2)    │
│     │                             │  参数: expPacket, pOsEvent              │
│     │                             │                                         │
│     │                             │  (驱动解析nodeId，发现是远程节点)         │
│     │                             │                                         │
│     │                             ▼                                         │
