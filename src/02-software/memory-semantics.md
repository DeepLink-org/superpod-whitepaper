...
本节回答的是统一内存子系统的第二个子问题：**这些能力如何被一个真实系统实例化？** NVIDIA 给出的答案并不是发明一套全新抽象，而是把上一节提出的语义承诺，拆解为一组可组合的工程模块：地址由 GMMU/PTE 承接，路由由 Fabric Base Address 与 NVSwitch 完成，协调由 Fabric Manager 与 IMEX 分层处理，编程接口则通过 Export / Import / Map 暴露给软件。
...
从"统一内存子系统"三节递进的视角看，本节证明了第 ① 节的通用语义框架并非抽象概念——它可以被拆解为 GMMU 页表配置、Fabric 地址编码、协调守护进程与分层软件接口等可实现的工程模块。但这仍然只回答了机制层问题：系统知道怎样让远端资源“可被正确访问”，还不等于知道这些资源“应该被怎样使用”。下一节"内存池化"将把视角拉升到策略层，讨论如何在多层级异构存储间高效调度，把统一寻址能力转化为实际的 Goodput 提升。
