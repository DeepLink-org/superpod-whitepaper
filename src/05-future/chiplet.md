...
超节点（Supernode）作为面向 AI 大模型训练、E 级科学计算、智算中心与自主可控算力基础设施的最高密度算力形态，其核心目标是在单机柜内实现"千卡级芯片、百 TB 级带宽、微秒级延迟"的一体化算力供给。在单芯片工艺逼近物理极限、良率成本急剧恶化、功耗密度持续突破上限的背景下，多 Die 堆叠（Multi-Die Stacking）已成为超节点架构中不可缺少、不可替代的底层支撑技术。

多 Die 堆叠，是指将多颗功能独立、工艺解耦的裸芯片（Die/Chiplet）通过 2.5D 中介层、3D 垂直键合、TSV、微凸点等先进封装手段，在物理空间上高密度集成，形成一个逻辑统一、带宽共享、延迟极低的"超级芯片"或"超级算力模块"。它不是 Chiplet 的子集，而是 Chiplet 得以规模化、高性能化、工程化落地的物理载体与实现基础。
