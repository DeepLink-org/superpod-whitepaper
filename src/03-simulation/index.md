...
当系统规模进入万卡级，且负载形态从"规则稠密训练"演进到"稀疏/MoE、长序列、多模态与在线推理混合运行"时，基础设施设计会出现一个现实困境：**试错成本过高**。一次硬件选型误判、一次并行策略或调度参数配置错误，都可能带来数周进度损失与巨额算力浪费。

因此，本章将建模仿真定位为 AI 基础设施的决策工具——用一套可复用的规约、模型与推演框架，把"争论"转化为"可验证的假设"，把"经验"转化为"可校准的资产"。内容从方法论与适用边界出发，经过负载建模、系统层仿真、性能资产库与校准流程，最终讨论仿真如何与真实系统迭代形成软硬协同闭环。仿真的价值不止于预测性能，更在于形成统一评估口径并为参考设计提供量化依据；但没有性能资产库、校准闭环和适用边界说明，仿真只会制造新的不确定性。

下一章将把这些方法论真正沉淀为参考设计，回答"如果今天要落地，一个方案该如何被选择与组织"。
...
因此，本白皮书将**帕累托前沿（Pareto Frontier）**作为建模分析的核心方法论之一：它不是给出唯一的“最好点”，而是把多目标权衡问题转化为一张可视化、可比较、可复用的“性能地图”。
